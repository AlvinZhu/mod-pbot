...
void AIWaypoint::NewToDatabase()
{
    ...
    WorldDatabase.PExecute("INSERT INTO aiwaypoints (entry, map, x, y, z, link) VALUES (%u, %u, %f, %f, %f, '%s')"
        , entry
        , mapID
        , posX
        , posY
...
void AIWaypoint::SaveToDatabase()
{
    ...
    WorldDatabase.PExecute("UPDATE aiwaypoints SET map = %u, x = %f, y = %f, z = %f, link = %s, helpText = %s WHERE entry = %u"
        , mapID
        , posX
        , posY
        , posZ
...
void AIWaypoint::DeleteToDatabase()
{
    ...
    WorldDatabase.PExecute("DELETE FROM aiwaypoints WHERE entry = %u", entry);
}
...
bool AIWaypointsMgr::LoadAIWaypoints()
{
    ...
    QueryResult curIncrementResult = CharacterDatabase.Query("SELECT `AUTO_INCREMENT` FROM information_schema.`TABLES` WHERE `TABLE_NAME`='playerbot_aiwaypoints'");
    ...
    QueryResult result = CharacterDatabase.Query("SELECT entry, map, x, y, z, link, helpText FROM playerbot_aiwaypoints");
    if (result)
    {
        do
        {
