    ...
    _navMeshQuery(pNavMeshQuery)
{
    ...
    TC_LOG_DEBUG("maps.mmaps", "++ Pathfinding::Pathfinding for %u", _pathParameter->unitGUID.GetCounter());
...
bool Pathfinding::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    TC_LOG_DEBUG("maps.mmaps", "++ Pathfinding::CalculatePath() for %u", _pathParameter->unitGUID.GetCounter());
...
void Pathfinding::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        if (waterPath)
        {
            ...
            {
                ZLiquidStatus status = _pathParameter->pBaseMap->GetLiquidStatus(_pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z, MAP_ALL_LIQUIDS, nullptr, _pathParameter->collisionHeight);
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            TC_LOG_ERROR("maps.mmaps", "%u's Path Build failed: 0 length path", _pathParameter->unitGUID.GetCounter());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
...
void Pathfinding::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        TC_LOG_DEBUG("maps.mmaps", "++ Pathfinding::BuildPointPath FAILED! path sized %d returned\n", pointCount);
        BuildShortcut();
    ...
    {
        TC_LOG_DEBUG("maps.mmaps", "++ Pathfinding::BuildPointPath FAILED! path sized %d returned, lower than limit set to %d", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    TC_LOG_DEBUG("maps.mmaps", "++ Pathfinding::BuildPointPath path type %d size %d poly-size %d", _type, pointCount, _polyLength);
}
...
void Pathfinding::UpdateAllowedPositionZ(float x, float y, float &z) const
{
    if (!_pathParameter->canFly)
    {
        ...
        float max_z;
        if (canSwim)
            max_z = _pathParameter->pMap->GetWaterOrGroundLevel(_pathParameter->phaseMask, x, y, z, &ground_z, !_pathParameter->hasAuraType);
        else
            max_z = ground_z = _pathParameter->pMap->GetHeight(_pathParameter->phaseMask, x, y, z, true);

        if (max_z > INVALID_HEIGHT)
        {
            // hovering units cannot go below their hover height
            float hoverOffset = _pathParameter->hoverOffset;
            max_z += hoverOffset;
    ...
    {
        float ground_z = _pathParameter->pMap->GetHeight(_pathParameter->phaseMask, x, y, z, true) + _pathParameter->hoverOffset;
        if (z < ground_z)
            z = ground_z;
...
{
    LiquidData data;
    ZLiquidStatus liquidStatus = _pathParameter->pBaseMap->GetLiquidStatus(x, y, z, MAP_ALL_LIQUIDS, &data, _pathParameter->collisionHeight);
...
bool Pathfinding::InRangeYZX(float const* v1, float const* v2, float r, float h) const
{
...
//void Pathfinding::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
//{
//    if (GetPathType() == PATHFIND_BLANK || _pathPoints.size() < 2)
//    {
//        TC_LOG_ERROR("maps.mmaps", "Pathfinding::ReducePathLengthByDist called before path was successfully built");
//        return;
//    }
//
...
//bool Pathfinding::IsInvalidDestinationZ(Unit const* target) const
//{
