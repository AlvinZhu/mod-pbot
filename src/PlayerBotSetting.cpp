...
void ItemsForLevel::AddItem(ItemTemplate const* pItem)
{
    ...
    if (IsTenacityItem(pItem))
    {
        ...
        //if (std::find(m_TenacityItems.begin(), m_TenacityItems.end(), pItem) == m_TenacityItems.end())
        //    m_TenacityItems.push_back(pItem);
        //else
        //    return;

        m_TenacityItems.insert(pItem->ItemId);
    }
    ...
    //if (std::find(m_Items.begin(), m_Items.end(), pItem) == m_Items.end())
    //    m_Items.push_back(pItem);

    m_Items.insert(pItem->ItemId);
}
...
bool PlayerBotSetting::IsBetterEquip(Player* player, const ItemTemplate* itemTemplate, int32 rndPropID)
{
    if (!itemTemplate || itemTemplate->RequiredLevel > player->GetLevel())
        return false;
    if (!IsEquipByClsAndTal(player->GetClass(), FindPlayerTalentType(player), itemTemplate, rndPropID))
        return false;
    uint16 eDest;
...
uint32 PlayerBotSetting::FindPlayerTalentType(Player* player)
{
    ...
    auto& playerTalents = sFakePlayerMgr->m_playerTalents;
    if (playerTalents.find(guid) != playerTalents.end())
        return playerTalents[guid].MaxPageIndex;
    ...
    PlayerBotTalent& playerTalent = playerTalents[guid];

    uint32 maxPageIndex = 0;
    uint32 maxPagePoint = 0;
    for (uint32 page = 0; page < 3; page++)
    {
        playerTalent.PageCount[page] = pageTalents[page];
    ...
    playerTalent.MaxPageIndex = maxPageIndex;
    playerTalent.MaxPagePoint = maxPagePoint;
...
void PlayerBotSetting::Initialize()
{
    ...
    {
        ItemTemplate const& item = itemTemplatePair.second;
...
void PlayerBotSetting::SupplementAmmo()
{
    ...
    if (count <= 0 || dest.empty())
        return;
    Item* itemInst = m_Player->StoreNewItem(dest, item->ItemId, true, GenerateItemRandomPropertyId(item->ItemId));
    if (itemInst)
...
void PlayerBotSetting::UpdateReset()
{
    ...
    case 13:
        ...
        m_Player->UpdateWeaponsSkillsToMaxSkillsForLevel();
...
{
    Trainer::Trainer const* trainer = sObjectMgr->GetTrainer(classesTrainersGUID[m_Player->GetClass()][(m_Player->GetTeamId() == TeamId::TEAM_ALLIANCE) ? 0 : 1]);
    if (!trainer)
        return;

    for (auto spell : trainer->GetSpells())
    {
        if (m_Player->HasSpell(spell.SpellId))
            m_Player->RemoveSpell(spell.SpellId, false, false);
    }

    //TrainerSpellData const* spellData = sObjectMgr->GetNpcTrainerSpells(classesTrainersGUID[m_Player->GetClass()][(m_Player->GetTeamId() == TeamId::TEAM_ALLIANCE) ? 0 : 1]);
    //if (!spellData)
    //	return;
...
void PlayerBotSetting::LearnSpells()
{
    ...
    for (auto spell : trainer->GetSpells())
    {
        ...
        if (spell.IsCastable())
        {
            m_Player->CastSpell(m_Player, spell.SpellId, true);
            TC_LOG_WARN("PlayerBotSetting", "Player %s Learn spell %d, spell is castable, do cast.", m_Player->GetName().c_str(), spell.SpellId);
        }
        else
            m_Player->LearnSpell(spell.SpellId, false);
    }
...
void PlayerBotSetting::AddEquipFromAll()
{
    ...
    if (prof <= 0 || prof >= MAX_CLASSES)
        return;
    ItemTemplate const* firstFinger = nullptr;
    ItemTemplate const* firstTrinket = nullptr;
...
void PlayerBotSetting::AddOnceEquip(ItemTemplate const* item)
{
    if (!item)
        return;
