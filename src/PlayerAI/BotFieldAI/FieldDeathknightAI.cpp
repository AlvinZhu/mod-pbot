...
{
    m_BotTalentType = FindTalentType(me);
}
...
void FieldDeathknightAI::ProcessMeleeSpell(Unit* pTarget)
{
    ...
    if (DKAttack_RuneAttack && TryCastSpell(DKAttack_RuneAttack, pTarget) == SpellCastResult::SPELL_CAST_OK)
        return;
    if (DKAttack_NearAOE && SpellHasReady(DKAttack_NearAOE))
    {
    ...
    if (DKIDLE_SummonAllPets && SpellHasReady(DKIDLE_SummonAllPets))
    {
        NearUnitVec& nearEnemys = RangeEnemyListByHasAura(0, NEEDFLEE_CHECKRANGE);
        if (nearEnemys.size() > 2)
        {
    ...
    if (DKAttack_AreaAOE && SpellHasReady(DKAttack_AreaAOE))
    {
...
{
    if (!DKBlock_Cast || !SpellHasReady(DKBlock_Cast))
        return false;
...
bool FieldDeathknightAI::ProcessMgcShield()
{
    ...
    if (DKDefense_MgcShield && SpellHasReady(DKDefense_MgcShield))
    {
        for (Unit* pUnit : enemys)
        {
            if (!pUnit->HasUnitState(UNIT_STATE_CASTING))
    ...
    if (m_BotTalentType == 2 && DKDefense_NoMgcArea && SpellHasReady(DKDefense_NoMgcArea))
    {
...
bool FieldDeathknightAI::ProcessDKPull(Unit* pTarget)
{
    if (!pTarget)
        return false;
    if (DKPulls_DKPull && SpellHasReady(DKPulls_DKPull))
    {
...
void FieldDeathknightAI::PetAction(Unit* pTarget)
{
    ...
    if (!pPet)
    {
        if (pTarget && DKIDLE_SummonPet && SpellHasReady(DKIDLE_SummonPet))
        {
