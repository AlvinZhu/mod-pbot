...
void BotFieldAI::UpdateBotAI(uint32 diff)//???????
{
    m_UpdateTick -= diff;
    if (m_UpdateTick > 0)
        return;
...
bool BotFieldAI::ProcessGroupInvite()
{
    if (Group* pGroup = me->GetGroup())
    {
        ...
        {
            m_Movement.ClearMovement();
            SelectPlayerBotAI(AITYPE_GROUP, true);
            return true;
        }
        //return false;
    }
...
bool BotFieldAI::IsBGSchedule()
{
    ...
    //    return true;
    if (me->InBattleground() || me->isUsingLfg())
        return true;
    return false;
}
...
bool BotFieldAI::CanSelectPlayerEnemy(Player* player)
{
    if (!player || player->GetTeamId() == me->GetTeamId() || IsNotSelect(player))
        return false;
    if (!m_DrivingPVP && !sFakePlayerMgr->FieldPVP && !player->IsPvP())
        return false;
    if (TargetIsStealth(player))
        return false;
    if (m_DrivingPVP || sFakePlayerMgr->FieldPVP)
        return true;
    else
    {
...
SpellCastResult BotFieldAI::TryCastSpell(uint32 spellID, Unit* pTarget, bool force, bool dismount)
{
    ...
    if (dismount)
        Dismount();
    SpellCastResult castResult = spell->prepare(targets, nullptr);
    //SpellCastResult castResult = DoCast(pTarget, spellID, force);
    if (castResult != SpellCastResult::SPELL_CAST_OK)
    {
        if (castResult == SpellCastResult::SPELL_FAILED_NOT_MOUNTED)
            PlayerBotSetting::ClearUnknowMount(me);
        else if (pTarget && castResult == SpellCastResult::SPELL_FAILED_BAD_TARGETS)
        {
            if (me->GetTarget() == pTarget->GetGUID())
                me->SetTarget(ObjectGuid::Empty);
            if (Player* playerTarget = pTarget->ToPlayer())
                if (!playerTarget->IsPlayerBot() && !playerTarget->IsPvP())
                {
                    WorldPacket opcode(1);
                    opcode << uint8(1);
                    playerTarget->GetSession()->HandleTogglePvP(opcode);
                }
        }
        //spell->finish(false);
        //delete spell;
...
void BotFieldAI::SearchCreatureListFromRange(Unit* center, NearCreatureVec& nearCreatures, float range, bool selfFaction)
{
    ...
    for (Creature* pCreature : nearCreature)
    {
        ...
        if (!selfFaction && !me->IsValidAttackTarget(pCreature))
            continue;
        if (sFakePlayerMgr->FieldCreature || selfFaction)
        {
