...
void BotShamanAI::ProcessFlee()
{
    ...
    if (ProcessTotem())
        return;
    if (ShamanAssist_Frog && !m_NeedReserveCtrlSpell && !TargetIsSuppress(pTarget) && SpellHasReady(ShamanAssist_Frog))
    {
        me->StopMoving();
...
float BotShamanAI::TryPushControlCommand(Player* pTarget)
{
    ...
    if (!TargetIsNotDiminishingByType2(pTarget, DIMINISHING_DISORIENT))
        return -1;
    if (!SpellHasReady(ShamanAssist_Frog))
        return -1;
...
bool BotShamanAI::ProcessTotem()
{
    ...
    {
        if (m_NonFearTotemTick > 0 || !ExistTotemByType(SUMMON_SLOT_TOTEM_EARTH, BOTAI_TOTEMRANGE))
        {
...
bool BotShamanAI::ExistTotemByType(SummonSlot slot, float range)
{
