...
bool BotPaladinAI::ProcessControl(Unit* pTarget)
{
    ...
    if (pTarget && PaladinAssist_StunMace && !TargetIsCtrl(pTarget) && SpellHasReady(PaladinAssist_StunMace))
    {
    ...
    if (PaladinAssist_StunMace && me->InArena() && SpellHasReady(PaladinAssist_StunMace))
    {
...
float BotPaladinAI::TryPushControlCommand(Player* pTarget)
{
    ...
    if (!TargetIsNotDiminishingByType2(pTarget, DIMINISHING_DISORIENT))
        return -1;
    if (!SpellHasReady(PaladinAssist_Confession, pTarget))
        return -1;
