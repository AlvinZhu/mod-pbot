...
BotBGAI* BotBGAI::debugBGAI = nullptr;
...
void BotBGAI::BattlegroundRevive()
{
    ...
    CommandBG* bgCommander = sPlayerBotBGMgr->GetCommander(player->GetMap(), player->GetTeamId());
    if (!bgCommander)
        return;
    ...
    if (m_UpdateTick <= 0)
    {
        ...
        if (IsAlive())
        {
            ...
            if (m_Teleporting.CanMovement())
                UpdateBotBGAI(BOTAI_UPDATE_TICK);
            else
...
bool BotBGAI::NeedDireFlee()
{
    if (m_CanDireFlee == 0)
    {
        if (Map* pMap = me->GetMap())
        {
            ...
            if (CommandBG* bgCommander = sPlayerBotBGMgr->GetCommander(pMap, me->GetTeamId()))
            {
...
void BotBGAI::UpdateBotBGAI(uint32 diff)
{
    Battleground* pBattleground = me->GetBattleground();
    if (!pBattleground)
        return;
    ...
    if (m_AIBGStateType == BotAIBGState::AIBGState_Ready)
    {
        ...
        {
            //CommandBG* pBGCommand = pBattleground->GetBgMap()->GetCommander(me->GetTeamId());
            CommandBG* pBGCommand = sPlayerBotBGMgr->GetCommander(pBattleground, me->GetTeamId());
            AIWaypoint* pAIWP = pBGCommand ? pBGCommand->GetReadyPosition() : nullptr;
            if (pAIWP)
                m_InitRndPos = pAIWP->GetPosition();
            else
                m_InitRndPos = me->GetPosition();
...
bool BotBGAI::TryUpMount()
{
    ...
    CommandBG* bgCommander = sPlayerBotBGMgr->GetCommander(me->GetMap(), me->GetTeamId());
...
bool BotBGAI::TargetIsMelee(Player* pTarget)
{
    ...
    {
    case CLASS_PALADIN:
        return (FindTalentType(pTarget) != 0);
    case CLASS_WARRIOR:
    case CLASS_ROGUE:
    case CLASS_DEATH_KNIGHT:
        return true;
    ...
    case CLASS_DRUID:
        return false;
    case CLASS_SHAMAN:
        return (FindTalentType(pTarget) == 1);
    }
    return false;
}
...
bool BotBGAI::TargetIsHealth(Player* pTarget)
{
    ...
    case CLASS_PALADIN:
        return (FindTalentType(pTarget) == 0);
    case CLASS_PRIEST:
        return (FindTalentType(pTarget) != 2);
    case CLASS_SHAMAN:
        return (FindTalentType(pTarget) == 2);
    case CLASS_DRUID:
        return (FindTalentType(pTarget) == 2);
...
bool BotBGAI::IsFleeTargetByRangeBot(Unit* pTarget)
{
    ...
    else if (Player* playerTarget = pTarget->ToPlayer())
        if (!playerTarget->IsPlayerBot())
        {
