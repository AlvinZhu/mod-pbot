    ...
    if (m_UpdateTick <= 0)
    {
        ...
        if (me->IsAlive())
        {
            if (m_Teleporting.CanMovement())
                UpdateBotDuelAI(BOTAI_UPDATE_TICK);
            else
...
{
    if (!me->duel || me->duel->State == DUEL_STATE_COMPLETED)
        return nullptr;

    Unit* pSelect = me->duel->Opponent;
    if (!pSelect)
        return nullptr;
...
void BotDuelAI::UpdateBotDuelAI(uint32 diff)
{
    EachTick();
    if (me->HasUnitState(UNIT_STATE_CASTING))
        return;
...
{
    if (!me->duel || me->duel->State == DUEL_STATE_COMPLETED || !me->duel->Opponent || !me->duel->Initiator)
    {
        m_Movement.ClearMovement();
        me->SetSelection(ObjectGuid::Empty);
        me->AttackStop();
...
{
    if (!me->duel || me->duel->State != DUEL_STATE_IN_PROGRESS || !me->duel->Opponent)
        return false;
