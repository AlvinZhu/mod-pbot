...
void DuelPriestAI::ProcessHealthSpell(Unit* pTarget)
{
    ...
    if (m_BotTalentType == 0 && PriestAssist_SoulAbs && manaPct < 40 && !me->HasAura(PriestAssist_SoulAbs) && SpellHasReady(PriestAssist_SoulAbs))
    {
        if (TryCastSpell(PriestAssist_SoulAbs, me) == SpellCastResult::SPELL_CAST_OK)
            return;
    }
    if (m_BotTalentType == 0 && PriestAssist_AddHolyPower && manaPct < 40 && !me->HasAura(PriestAssist_AddHolyPower) && SpellHasReady(PriestAssist_AddHolyPower))
    {
