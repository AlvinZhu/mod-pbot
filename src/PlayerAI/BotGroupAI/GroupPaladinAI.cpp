...
bool GroupPaladinAI::ProcessTryFriendImmune()
{
    if (ProcessTryFullHealth())
        return true;
    if (!PaladinGuard_PhyImmune || SpellHasReady(PaladinGuard_PhyImmune))
        return false;
...
{
    if (!PaladinHeal_FullHoly || SpellHasReady(PaladinHeal_FullHoly))
        return false;
...
bool GroupPaladinAI::ProcessFullDispel()
{
    ...
    if (PaladinAssist_Dispel != 0)
    {
        if (ProcessDispel())
            return true;
        else if (!SpellHasReady(PaladinAssist_Dispel))
            return true;
    }
...
bool GroupPaladinAI::ProcessStamp()
{
    ...
    if (manaPct < 15)
    {
        ...
        if (PaladinAssist_ManaStamp && !me->HasAura(PaladinAssist_ManaStamp) && TryCastSpell(PaladinAssist_ManaStamp, me) == SpellCastResult::SPELL_CAST_OK)
            return true;
        if (PaladinMelee_ManaJudge && SpellHasReady(PaladinMelee_ManaJudge))
        {
...
{
    if (PaladinAssist_Dispel == 0 || !SpellHasReady(PaladinAssist_Dispel))
        return false;
    NearUnitVec& friends = SearchFriend(BOTAI_RANGESPELL_DISTANCE);
    if (friends.empty())
        return false;
