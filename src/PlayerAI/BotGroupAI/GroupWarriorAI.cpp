...
void GroupWarriorAI::ProcessWeaponMeleeSpell(Unit* pTarget)
{
    ...
    if (TryCastSpell(WarriorWeaponRage_WinAttack, pTarget) == SpellCastResult::SPELL_CAST_OK)
        return;
    if (WarriorRage_HeadAtt && pTarget->HasUnitState(UNIT_STATE_CASTING) && SpellHasReady(WarriorRage_HeadAtt))
    {
        if (!me->HasAura(WarriorRage_Status))
            me->CastSpell(me, WarriorRage_Status, true);
...
{
    if (!WarriorWeapon_Backstorm || !SpellHasReady(WarriorWeapon_Backstorm))
        return false;
...
bool GroupWarriorAI::ProcessFullAttack(Unit* pTarget)
{
    ...
    if (WarriorRage_Needdead && targetMeEnemys.size() <= 1 && SpellHasReady(WarriorRage_Needdead))
    {
        if (!me->HasAura(WarriorRage_Status))
            me->CastSpell(me, WarriorRage_Status, true);
    ...
    if (WarriorRage_Impertinency && targetMeEnemys.empty() && SpellHasReady(WarriorRage_Impertinency))
    {
        if (!me->HasAura(WarriorRage_Status))
            me->CastSpell(me, WarriorRage_Status, true);
