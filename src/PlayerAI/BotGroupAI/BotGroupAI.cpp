...
BotGroupAI* BotGroupAI::debugGroupAI = nullptr;
...
void BotGroupAI::ProcessListEquip(Player* srcPlayer)
{
    ...
    for (uint8 slot = InventoryPackSlots::INVENTORY_SLOT_ITEM_START; slot < InventoryPackSlots::INVENTORY_SLOT_ITEM_END; slot++)
    {
        ...
        WorldPackets::Query::QueryItemSingle request(std::move(cmd));
        pSession->HandleItemQuerySingleOpcode(request);
    }
...
void BotGroupAI::ProcessUpequip(Player* srcPlayer, std::string& equipLink)
{
    ...
    else
    {
        ...
        if (msg != EQUIP_ERR_OK)
        {
            ...
            me->Whisper("装备失败", Language::LANG_COMMON, srcPlayer);
            return;
        }
    ...
    me->Whisper("装备成功", Language::LANG_COMMON, srcPlayer);
}
...
void BotGroupAI::ProcessUnequip(Player* srcPlayer, std::string& equipLink)
{
    ...
    for (uint8 slot = EquipmentSlots::EQUIPMENT_SLOT_HEAD; slot < EquipmentSlots::EQUIPMENT_SLOT_END; slot++)
    {
        ...
        if (msg != EQUIP_ERR_OK)
        {
            ...
            me->Whisper("取下失败", Language::LANG_COMMON, srcPlayer);
            return;
        }
        ...
        me->Whisper("成功取下", Language::LANG_COMMON, srcPlayer);
        return;
    }
    //std::string outString;
    //consoleToUtf8(std::string("取下失败"), outString);
...
void BotGroupAI::ProcessDestroyItem(Player* srcPlayer, std::string& equipLink)
{
    ...
    me->Whisper("成功丢掉", Language::LANG_COMMON, srcPlayer);
}
...
void BotGroupAI::ProcessTradeItem(Player* srcPlayer, std::string& equipLink)
{
    ...
    if (entry == 0)
    {
        ...
        me->Whisper("我没有这个道具", Language::LANG_COMMON, srcPlayer);
        return;
    }
    ...
    if (!pTrade)
    {
        ...
        me->Whisper("没有开始交易", Language::LANG_COMMON, srcPlayer);
        return;
    }
    ...
    if (pTrade->HasItem(pItem->GetGUID()))
    {
        ...
        me->Whisper("这个道具已经放上去了", Language::LANG_COMMON, srcPlayer);
        return;
    }
    ...
    if (pTrade->SetItemAtNullSlot(pItem))
    {
        ...
        me->Whisper("放上去了", Language::LANG_COMMON, srcPlayer);
    }
    else
    {
        //std::string outString;
        //consoleToUtf8(std::string("没法再放东西了"), outString);
        me->Whisper("没法再放东西了", Language::LANG_COMMON, srcPlayer);
    }
}
...
void BotGroupAI::ProcessUseItem(Player* srcPlayer, std::string& equipLink)
{
    ...
    if (me->CastItemUseSpell(pItem, targets, 0, 0) == SpellCastResult::SPELL_CAST_OK)
    {
        ...
        me->Whisper("使用失败", Language::LANG_COMMON, srcPlayer);
        return;
    }
    //std::string outString;
    //consoleToUtf8(std::string("成功使用"), outString);
    me->Whisper("使用成功", Language::LANG_COMMON, srcPlayer);
}
...
void BotGroupAI::ProcessTalent(Player* srcPlayer, std::string& talentText)
{
    ...
    me->Whisper("天赋切换完成", Language::LANG_COMMON, srcPlayer);
#endif
}
...
void BotGroupAI::ProcessSummonRiteSpell(Player* srcPlayer)
{
    ...
    if (!m_MovetoUseGO.CanCastSummonRite())
    {
        ...
        me->Whisper("目前无法开始召唤仪式！", Language::LANG_COMMON, srcPlayer);
        return;
    }
    ...
    if (castSpellID)
    {
        ...
        me->Whisper("召唤仪式启动！", Language::LANG_COMMON, srcPlayer);
    }
    else
    {
        //std::string outString;
...
void BotGroupAI::UpdateBotAI(uint32 diff)
{
    ...
    if (TryTeleportToMaster())
        return;
    if (m_MasterPlayer && m_MasterPlayer->duel && m_MasterPlayer->duel->State != DUEL_STATE_COMPLETED)
    {
        if (!me->duel)
        {
    ...
    else
    {
        ...
        if (m_SeduceTarget != ObjectGuid::Empty)
        {
            ...
            if (Group* pGroup = me->GetGroup())
                ClearAllGroupForceFleeState(pGroup);
        }
...
bool BotGroupAI::UpdateMasterPlayer()
{
    ...
    if (!pGroup)
    {
        ...
        SelectPlayerBotAI(AITYPE_FIELD, true);
        return false;
    }
    if (!GroupExistRealPlayer(pGroup))
    {
        m_Flying.CancelFly();
        sLFGMgr->LeaveLfg(me->GetGUID());
        return false;
...
Unit* BotGroupAI::GetCombatTarget(float range)
{
    ...
    if (!IsTankBotAI())
    {
        Unit* pTankTarget = GetGroupTankTarget(pGroup);
...
void BotGroupAI::ProcessFollowToMaster()
{
    ...
    if (m_MasterPlayer->duel && m_MasterPlayer->duel->State != DUEL_STATE_COMPLETED)
    {
        m_Movement.ClearMovement();
        return;
    }
...
bool BotGroupAI::NonCombatProcess()
{
    m_DelayGiveXP.ProcessGiveXP(m_MasterPlayer->GetLevel());
    {
        ...
        if (m_CheckDuel.CheckDuel())
            SelectPlayerBotAI(AITYPE_DUEL, true);
...
bool BotGroupAI::DoFaceToTarget(Unit* pTarget)
{
    ...
    if (relative >= M_PI_2 && !IsNotMovement())// (fabsf(selfAngle) > M_PI_4)
    {
        ...
        me->SetFacingToObject(pTarget, false);
        return true;
    }
    return false;
}
...
SpellCastResult BotGroupAI::PetTryCastSpell(uint32 spellID, Unit* pTarget, bool force)
{
    ...
    else
    {
        ...
        if (!pPet->HasSpellCooldown(spellID))
            pPet->ResetSpellCooldown(spellID, true);
    ...
    if (distance <= 5.0f)
    {
        if (!HasAuraMechanic(pUnit, Mechanics::MECHANIC_FEAR) && !HasAuraMechanic(pUnit, Mechanics::MECHANIC_HORROR) && !HasAuraMechanic(pUnit, Mechanics::MECHANIC_STUN))
        {
            ...
            if (guid.IsPlayer())
            {
                ...
                {
                    if (!IsTankPlayer(pUnitTarget) || pUnitTarget->GetTarget() != pUnit->GetGUID())
