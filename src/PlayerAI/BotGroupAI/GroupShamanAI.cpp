...
bool GroupShamanAI::ProcessFullDispel()
{
    ...
    {
        if (!SpellHasReady(ShamanDispel_Refine))
            return true;
    }
    if (ShamanAssist_DecCruel != 0)
    {
        if (!SpellHasReady(ShamanAssist_DecCruel))
            return true;
    }
...
{
    if (ShamanDispel_Refine == 0 || !SpellHasReady(ShamanDispel_Refine))
        return false;
...
{
    if (ShamanAssist_DecCruel == 0 || !SpellHasReady(ShamanAssist_DecCruel))
        return false;
    NearUnitVec& friends = SearchFriend(BOTAI_RANGESPELL_DISTANCE);
    if (friends.empty())
        return false;
...
{
    if (ShamanTotem_DecMove && !ExistTotemByType(SUMMON_SLOT_TOTEM_EARTH, BOTAI_TOTEMRANGE))
    {
        if (TryCastSpell(ShamanTotem_DecMove, me) == SpellCastResult::SPELL_CAST_OK)
            return true;
    }
    if (ShamanTotem_AbsorbBuff && !ExistTotemByType(SUMMON_SLOT_TOTEM_AIR, BOTAI_TOTEMRANGE))
    {
...
{
    if (!ExistTotemByType(SUMMON_SLOT_TOTEM_WATER, BOTAI_TOTEMRANGE))
    {
    ...
    if (!ExistTotemByType(SUMMON_SLOT_TOTEM_FIRE, BOTAI_TOTEMRANGE))
    {
    ...
    if (!ExistTotemByType(SUMMON_SLOT_TOTEM_AIR, BOTAI_TOTEMRANGE))
    {
...
bool GroupShamanAI::ProcessMeleeStateTotem()
{
    if (m_BotTalentType != 1)
        return false;
    if (ShamanTotem_Life && !ExistTotemByType(SUMMON_SLOT_TOTEM_WATER, BOTAI_TOTEMRANGE))
    {
        if (TryCastSpell(ShamanTotem_Life, me) == SpellCastResult::SPELL_CAST_OK)
            return true;
    }
    ...
    {
        bool existFireTotem = ExistTotemByType(SUMMON_SLOT_TOTEM_FIRE, BOTAI_TOTEMRANGE);
    ...
    if (!ExistTotemByType(SUMMON_SLOT_TOTEM_EARTH, BOTAI_TOTEMRANGE))
    {
...
bool GroupShamanAI::ExistTotemByType(SummonSlot slot, float range)
{
    ...
    for (Creature* pObject : results)
    {
        ...
        if (!pTotem || pTotem->GetOwner() != me)
            continue;
        if (pTotem->m_Properties->Slot != slot)
            continue;
        return true;
    }
    return false;
