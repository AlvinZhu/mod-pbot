...
bool GroupPriestAI::ProcessFullDispel()
{
    ...
    {
        if (!SpellHasReady(PriestAssist_Dispel))
            return true;
    }
    if (PriestAssist_DecIllness != 0)
    {
        if (!SpellHasReady(PriestAssist_DecIllness))
            return true;
    }
...
{
    if (PriestAssist_Dispel == 0 || !SpellHasReady(PriestAssist_Dispel))
        return false;
...
{
    if (PriestAssist_DecIllness == 0 || !SpellHasReady(PriestAssist_DecIllness))
        return false;
    NearUnitVec& friends = SearchFriend(BOTAI_RANGESPELL_DISTANCE);
    if (friends.empty())
        return false;
