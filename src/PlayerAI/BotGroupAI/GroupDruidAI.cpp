...
bool GroupDruidAI::ProcessFullDispel()
{
    ...
    {
        if (!SpellHasReady(DruidAssist_DecCruse))
            return true;
    }
    if (DruidAssist_DecCruel != 0)
    {
        if (!SpellHasReady(DruidAssist_DecCruel))
            return true;
    }
...
{
    if (DruidAssist_DecCruse == 0 || !SpellHasReady(DruidAssist_DecCruse))
        return false;
...
{
    if (DruidAssist_DecCruel == 0 || !SpellHasReady(DruidAssist_DecCruel))
        return false;
    NearUnitVec& friends = SearchFriend(BOTAI_RANGESPELL_DISTANCE);
    if (friends.empty())
        return false;
...
void GroupDruidAI::ProcessCombatRevive()
{
    ...
    if (!pGroup)
        return;
    std::vector<ObjectGuid>& needPlayers = GetGroupMemberFromNeedRevivePlayer(pGroup, me->GetMapId());
